...
export function FaturamentoEntradas() {
  ...
  const loadData = async () => {
    ...
    if (filtroMes) {
      ...
      // Assumes generic end of month parsing logic
  ...
  useEffect(() => {
    loadData()
  }, [filtroMes, filtroTipo, buscaNome])
  ...
  const exportCSV = () => {
    let csv = 'Data,Paciente,Tipo,Descrição,Valor,Forma de Pagamento,Observações\n'
    lancamentos.forEach((l) => {
      csv += `${l.data_atendimento},"${l.nome_paciente}",${l.tipo},"${l.descricao || ''}",${l.valor},${l.forma_pagamento},"${l.observacoes || ''}"\n`
    })
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col md:flex-row gap-4 items-end bg-card p-5 rounded-2xl border border-border/60 shadow-sm print:hidden">
