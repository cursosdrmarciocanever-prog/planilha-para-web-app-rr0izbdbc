...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (phoneDigits.length >= 8) {
      ...
      if (agendamentos) {
        agendamentoContext = agendamentos.find(a => 
    ...
    if (templates && templates.length > 0) {
      if (textoMsg.includes('cancelar') || textoMsg.includes('desmarcar')) {
        templateMatch = templates.find(t => t.categoria.toLowerCase() === 'cancelamento' || t.titulo.toLowerCase().includes('cancel'))
      } else if (textoMsg.includes('confirmar')) {
        templateMatch = templates.find(t => t.categoria.toLowerCase() === 'confirmação' || t.titulo.toLowerCase().includes('confirm'))
      } else if (textoMsg.includes('adiar') || textoMsg.includes('remarcar')) {
        templateMatch = templates.find(t => t.categoria.toLowerCase() === 'adiamento' || t.titulo.toLowerCase().includes('adiar') || t.titulo.toLowerCase().includes('remarcar'))
      } else if (textoMsg.includes('horário') || textoMsg.includes('horas') || textoMsg.includes('funcionamento')) {
        templateMatch = templates.find(t => t.titulo.toLowerCase().includes('horário') || (t.categoria.toLowerCase() === 'dúvidas frequentes' && t.conteudo.toLowerCase().includes('horário')))
      } else if (textoMsg.includes('endereço') || textoMsg.includes('local') || textoMsg.includes('onde')) {
        templateMatch = templates.find(t => t.titulo.toLowerCase().includes('endereço') || t.titulo.toLowerCase().includes('local') || (t.categoria.toLowerCase() === 'dúvidas frequentes' && t.conteudo.toLowerCase().includes('endereço')))
      } else if (textoMsg.includes('valor') || textoMsg.includes('preço') || textoMsg.includes('custa')) {
        templateMatch = templates.find(t => t.titulo.toLowerCase().includes('valor') || t.titulo.toLowerCase().includes('preço') || (t.categoria.toLowerCase() === 'dúvidas frequentes' && t.conteudo.toLowerCase().includes('valor')))
      } else if (textoMsg.includes('preparo') || textoMsg.includes('preparação')) {
        templateMatch = templates.find(t => t.titulo.toLowerCase().includes('preparo') || (t.categoria.toLowerCase() === 'dúvidas frequentes' && t.conteudo.toLowerCase().includes('preparo')))
      } else if (textoMsg.includes('agendar') || textoMsg.includes('marcar')) {
        templateMatch = templates.find(t => t.categoria.toLowerCase() === 'agendamento' || t.titulo.toLowerCase().includes('agendar') || t.titulo.toLowerCase().includes('marcar'))
      } else if (textoMsg.includes('dúvida')) {
        templateMatch = templates.find(t => t.categoria.toLowerCase() === 'dúvidas frequentes' || t.titulo.toLowerCase().includes('dúvida'))
      }
    }
    ...
    if (templateMatch) {
      ...
      if (agendamentoContext?.data_hora) {
        ...
        horaConsulta = dh.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })
      }
